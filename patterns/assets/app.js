...
(function () {
  ...
  /* ---------- Detect language from the URL (/patterns/mr/... = Marathi) ---------- */
  var path = location.pathname.replace(/index\.html$/, '');
  if (path.charAt(path.length - 1) !== '/') path += '/';
  var onAi = path.indexOf('/patterns/') === 0;
  var isMr = path.indexOf('/patterns/mr/') === 0;
  var T = isMr ? {
  ...
  if (onAi) {
    ...
    if (navWrap) {
      var enPath = isMr ? path.replace('/patterns/mr/', '/patterns/') : path;
      var mrPath = isMr ? path : path.replace('/patterns/', '/patterns/mr/');
